...
/* Static snapshot mode (e.g. Vercel): the same UI reads files exported by scripts/export_static.py. */
const STATIC = window.FG_STATIC === true;
const localApprovals = [];

function staticPath(path) {
  const m = path.match(/^\/api\/cases\/([^/]+)(?:\/(graph|execution))?$/);
  if (path === "/api/health") return "data/health.json";
  if (path === "/api/overview") return "data/overview.json";
  if (path === "/api/cases") return "data/cases.json";
  if (path === "/api/monitor") return "data/monitor.json";
  if (m) return `data/${m[2] || "case"}/${encodeURIComponent(m[1])}.json`;
  return null;
}

async function staticApi(path, opts) {
  const approve = path.match(/^\/api\/cases\/([^/]+)\/approve$/);
  if (approve) return staticApprove(approve[1], JSON.parse(opts.body));
  const file = staticPath(path);
  const r = file ? await fetch(file) : null;
  if (!r || !r.ok) throw new Error("not investigated yet");
  const body = await r.json();
  if (path.endsWith("/execution")) {
    return body.map((a) => {
      const done = localApprovals.find((x) => x.case_id === decodeURIComponent(path.split("/")[3]) && x.action === a.action);
      return done ? { ...a, state: `${done.decision}d by ${done.role} (demo, not saved)` } : a;
    });
  }
  return body;
}

async function staticApprove(caseId, d) {
  // same checks as the live API (fraudagent/api/server.py)
  const t = await staticApi(`/api/cases/${caseId}`);
  const rec = t.next_best_actions.final.find((a) => a.action === d.action);
  if (!rec) throw new Error(`${d.action} is not a recommended action for ${caseId}`);
  if (rec.route === "auto") throw new Error(`${d.action} is an auto action; the agent executes it without approval`);
  if (rec.route === "L2" && d.role !== "L2") throw new Error(`${d.action} requires a fraud manager (L2); ${d.role} cannot approve it`);
  if (localApprovals.some((a) => a.case_id === caseId && a.action === d.action)) throw new Error(`${d.action} was already decided`);
  const entry = { case_id: caseId, action: d.action, route: rec.route, decision: d.decision, role: d.role, at: new Date().toISOString() };
  localApprovals.push(entry);
  return entry;
}
...
function renderHealth(h) {
  ...
  $("#sys").innerHTML = `<span class="pill"><i></i>TigerGraph · ${esc(h.graph_backend)}</span>${llm}`
    + (STATIC ? `<span class="pill off" title="Hosted snapshot of the real TigerGraph run. Live runs need the backend."><i></i>read-only snapshot</span>` : "");
}
...
async function runAdhoc(e) {
  e.preventDefault();
  if (STATIC) {
    $("#adhocMsg").textContent = "This hosted page is a snapshot of the TigerGraph run. Ad-hoc investigations need the live backend (see the README).";
    return;
  }
...
async function openCase(id) {
  ...
  $("#runBtn").disabled = !/^HHG-/.test(id) && !STATIC;
  if (STATIC) $("#runBtn").textContent = "Replay investigation";
...
function replay(id) {
  // snapshot mode: replay the recorded agent run step by step
  const btn = $("#runBtn");
  const events = (state.trace && state.trace.timeline) || [];
  ...
  const step = () => {
    if (i >= events.length) {
      addEvent({ step: "done", detail: "Replay of the recorded TigerGraph run complete" });
      renderStepper(events); btn.disabled = false; btn.textContent = "Replay again";
      return;
    }
    const e = events[i++];
    live.push(e); addEvent(e); renderStepper(live, true);
    setTimeout(step, e.step === "tool" ? 380 : 220);
  };
  step();
}
